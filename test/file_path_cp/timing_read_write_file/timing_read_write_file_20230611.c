...
void get_local_time(char *timeStr)
{
    ...
    strftime(buffer, LOG_TIME_BUF_SIZE, "%Y-%m-%d %H:%M:%S", ptm); //输出格式为: 2018-12-09 10:52:57.200
...
char readFile(char *str)
{
	...
	int n = read(fd, str, MAX_STRINT_SIZE);
	printf("n: %d\nread: {%s}\n", n, str);
	if(n < 0)
	{
		printf("read failed!\n");
...
char writeFile(int times, char *str)
{
	if(NULL == str)
	{
		printf("NULL == str!\n");
...
char checkFileModified(unsigned long *lastModifiedTime)
{
	struct stat buf;
	...
	{
		//printf(">>>> st_mtime: %s, st_mtime: %lu, lastModifiedTime: %lu\n", ctime(&buf.st_mtime), buf.st_mtime, *lastModifiedTime);
...
int main()
{
	...
	int times = 1;
	do{
		char cRet = checkFileModified(&lastModifiedTime);
		if(cRet < 0)
		{
