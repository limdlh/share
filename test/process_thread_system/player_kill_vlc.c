...
void *ThreadFun(void *arg)
{
	...
	//if(system("killall vlc -e") == -1){
		printf("execl filed!\n");
		perror("becasue");
	}else{
		printf("else case of [system(\"pkill ffplay\")]\n");
	}
	printf("---------------------------------->\n");
...
int main()
{
	...
	//if(system("vlc rtsp://192.168.124.3:554/media/video3") == -1){
	//if(system("ffplay -rtsp_transport tcp rtsp://192.168.124.3:554/media/video3") == -1){
		printf("execl filed!\n");
		perror("becasue");
	}else{
		printf("else case of [system(\"ffplay\")]\n");
	}
	printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
