...
int redis_pool_create(const REDIS_CONFIG* config, REDIS_INSTANCE** instance)
{
    ...
    if (config->endpoints == NULL || config->num_endpoints < 1) {
		...
		ERROR("%s: Must provide 1 redis endpoint", __func__);
    ...
    if (inst->config->num_redis_socks > MAX_REDIS_SOCKS) {
	    ...
	    //ERROR("%s: Number of redis sockets (%d) cannot exceed MAX_REDIS_SOCKS (%d)",
        //       __func__,
        //       inst->config->num_redis_socks, MAX_REDIS_SOCKS);
        redis_pool_destroy(inst);
        return -1;
    ...
    for (i = 0; i < inst->config->num_endpoints; i++) {
        ...
        if (host == NULL || strlen(host) == 0 || port <= 0 || port > 65535) {
			...
			//ERROR("%s: Invalid redis endpoint @%d: %s:%d", __func__, i, host, port);
            redis_pool_destroy(inst);
            return -1;
        }
        //log_(L_INFO, "%s: Got redis endpoint @%d: %s:%d", __func__, i, host, port);
		//INFO("%s: Got redis endpoint @%d: %s:%d", __func__, i, host, port);
    }
    ...
    //INFO("%s: Attempting to connect to above endpoints with connect_timeout %d net_readwrite_timeout %d",
    //    __func__,
    //    inst->config->connect_timeout, inst->config->net_readwrite_timeout);
...
static int redis_init_socketpool(REDIS_INSTANCE * inst)
{
    ...
    for (i = 0; i < inst->config->num_redis_socks; i++) {
        ...
        if (rcode != 0) {
			...
			//ERROR("%s: Failed to init lock: returns (%d)", __func__, rcode);
            free(redisocket);
            return -1;
        }
    ...
    if (!success) {
		...
		//WARN("%s: Failed to connect to any redis server.", __func__);
    }
...
static int connect_single_socket(REDIS_SOCKET *redisocket, REDIS_INSTANCE *inst)
{
    ...
    for (i = 0; i < inst->config->num_endpoints; i++) {
        ...
        passwd=inst->config->endpoints[redisocket->backup].password;
        idxDB=inst->config->endpoints[redisocket->backup].db;
        ...
        if (c && c->err == 0) {
            ...
            if (redisSetTimeout(c, timeout[1]) != REDIS_OK) {
				...
				//WARN("%s: Failed to set timeout: blocking-mode: %d, %s",
                //    __func__, (c->flags & REDIS_BLOCK), c->errstr);
            }
            ...
            if (redisEnableKeepAlive(c) != REDIS_OK) {
                ...
                //    __func__, c->errstr);
				//WARN("%s: Failed to enable keepalive: %s",
                //    __func__, c->errstr);
            }
            //redis auth
            if(passwd != NULL && strlen(passwd) > 1){//db auth
                string cmdbuf=format("auth %s", passwd);
				redisReply* reply_auth=NULL;
				reply_auth = (redisReply*)redis_command(redisocket, inst, cmdbuf.c_str());
                if(reply_auth==NULL){
                    break;
                }else if(reply_auth->type == REDIS_REPLY_ERROR){
                	freeReplyObject(reply_auth);
					break;
				}else if(reply_auth->type == REDIS_REPLY_STATUS){
					if(strncmp(reply_auth->str,"OK",2)){
						freeReplyObject(reply_auth);
						break;
					}
				}
				if(reply_auth != NULL) freeReplyObject(reply_auth);
            }
			//redis db select
			if(idxDB >= 0){ //select db index
				string cmdbuf=format("select %d", idxDB);
				redisReply* reply_sel=NULL;
				reply_sel = (redisReply*)redis_command(redisocket, inst, cmdbuf.c_str());
				if(reply_sel==NULL){
					break;
				}else if(reply_sel->type == REDIS_REPLY_ERROR){
					freeReplyObject(reply_sel);
					break;
				}else if(reply_sel->type == REDIS_REPLY_STATUS){
					if(strncmp(reply_sel->str,"OK",2)){
						freeReplyObject(reply_sel);
						break;
					}
				}
				if(reply_sel != NULL) freeReplyObject(reply_sel);
			}
        ...
        if (c) {
			...
			//WARN("%s: Failed to connect redis handle #%d @%d: %s, trying backup",
            //    __func__, redisocket->id, redisocket->backup, c->errstr);
            redisFree(c);
        } else {
            //log_(L_WARN|L_CONS, "%s: can't allocate redis handle #%d @%d, trying backup",
            //    __func__, redisocket->id, redisocket->backup);
			//WARN("%s: can't allocate redis handle #%d @%d, trying backup",
            //    __func__, redisocket->id, redisocket->backup);
        }
    ...
    if (c) {
        ...
        //    __func__, redisocket->id, redisocket->backup, c->errstr);
		//WARN("%s: Failed to connect redis handle #%d @%d: %s",
        //    __func__, redisocket->id, redisocket->backup, c->errstr);
        redisFree(c);
    } else {
        //log_(L_WARN|L_CONS, "%s: can't allocate redis handle #%d @%d",
        //    __func__, redisocket->id, redisocket->backup);
		//WARN("%s: can't allocate redis handle #%d @%d",
        //    __func__, redisocket->id, redisocket->backup);
    }
    redisocket->conn = NULL;
    redisocket->state = sockunconnected;
...
static int redis_close_socket(REDIS_INSTANCE *inst, REDIS_SOCKET * redisocket)
{
    ...
    if (redisocket->inuse) {
		...
		//FATAL("%s: I'm still in use. Bug?", __func__);
    }
    ...
    if(rcode != 0) {
        ...
        //    __func__, rcode);
		//WARN("%s: Failed to destroy lock: returns (%d)",
        //    __func__, rcode);
    }
...
REDIS_SOCKET * redis_get_socket(REDIS_INSTANCE * inst)
{
    ...
    while (cur) {
        ...
        if ((rcode = pthread_mutex_trylock(&cur->mutex)) != 0) {
            ...
            locked = 1;
            //DEBUG("%s: Obtained lock with handle %d", __func__, cur->id);
        }
        ...
        if(cur->inuse == 1) {
            if (locked) {
                if((rcode = pthread_mutex_unlock(&cur->mutex)) != 0) {
					...
					//FATAL("%s: Can not release lock with handle %d: returns (%d)",
                    ;//    __func__, cur->id, rcode);
                } else {
                    ;//DEBUG("%s: Released lock with handle %d", __func__, cur->id);
                }
            }
            goto next;
        }
        ...
        if ((cur->state == sockunconnected) && (time(NULL) > inst->connect_after)) {
            ...
            //       __func__, cur->id);
			//INFO("%s: Trying to (re)connect unconnected handle %d ...",
            //       __func__, cur->id);
            tried_to_connect++;
        ...
        if (unconnected != 0 || tried_to_connect != 0) {
			...
			//INFO("%s: got socket %d after skipping %d unconnected handles, "
            //       "tried to reconnect %d though",
            //       __func__, cur->id, unconnected, tried_to_connect);
        }
	...
	//WARN("%s: There are no redis handles to use! skipped %d, tried to connect %d",
    //       __func__, unconnected, tried_to_connect);
    return NULL;
}
...
int redis_release_socket(REDIS_INSTANCE * inst, REDIS_SOCKET * redisocket)
{
    ...
    if (redisocket->inuse != 1) {
		...
		//FATAL("%s: I'm NOT in use. Bug?", __func__);
    }
    redisocket->inuse = 0;
    ...
    if((rcode = pthread_mutex_unlock(&redisocket->mutex)) != 0) {
        ...
        //DEBUG("%s: Released lock with handle %d", __func__, redisocket->id);
    }
...
void* redis_vcommand(REDIS_SOCKET* redisocket, REDIS_INSTANCE* inst, const char* format, va_list ap)
{
    ...
    if (reply == NULL) {
        ...
        if (connect_single_socket(redisocket, inst) < 0) {
			...
			//ERROR("%s: Reconnect failed, server down?", __func__);
            goto quit;
        }
        ...
        if (reply == NULL) {
			...
			//ERROR("%s: Failed after reconnect: %s (%d)", __func__, c->errstr, c->err);
